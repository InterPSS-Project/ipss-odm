...
public class PSSEHeaderDataRawMapper extends BasePSSEDataRawMapper {
	
	public PSSEHeaderDataRawMapper(PsseVersion ver) {
		super(ver);
		this.dataParser = new PSSEHeaderDataRawParser(ver);
	}
	...
	public void procLineString(String[] lineStrAry, BaseAclfModelParser<? extends NetworkXmlType> parser) throws ODMException {
		...
		if (PSSERawAdapter.getVersionNo(this.version) >= 31) {
	 	   ...
	 	   */
			...
			String str = new Integer(PSSERawAdapter.getVersionNo(this.version)).toString();
			if (!ver.contains(str)) {
